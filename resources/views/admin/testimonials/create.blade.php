...
@section('content')
    <div class="max-w-2xl">
        <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-2xl shadow-sm">
            @csrf
            <div>
            ...
            <div>
                ...
                <textarea name="message" rows="4" required class="w-full border border-slate-200 rounded-xl px-4 py-2">{{ old('message') }}</textarea>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="url" name="photo_url" value="{{ old('photo_url') }}" class="w-full border border-slate-200 rounded-xl px-4 py-2">
            </div>
            <div>

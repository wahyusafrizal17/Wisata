    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div>
                ...
                @php($waShow = whatsapp_display_number($contact['whatsapp'] ?? ''))
                <ul class="space-y-3 text-sm">
                    <li>
                        <a href="{{ whatsapp_contact_href($contact['whatsapp'] ?? '') }}" target="_blank" rel="noopener noreferrer" class="flex items-start gap-3 text-slate-600 hover:text-brand-700 transition-colors group">
                            <span class="shrink-0 w-9 h-9 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center group-hover:bg-emerald-100" aria-hidden="true">
                            ...
                            <span class="min-w-0 pt-0.5">
                                <span class="block text-xs font-medium text-slate-500 uppercase tracking-wide">WhatsApp</span>
                                <span class="text-slate-700 group-hover:text-brand-700 break-all">{{ $waShow !== '' ? $waShow : '—' }}</span>
                            </span>
                        </a>
                    </li>
                    <li>
                        <a href="mailto:{{ $contact['email'] ?? '' }}" class="flex items-start gap-3 text-slate-600 hover:text-brand-700 transition-colors group">
                            <span class="shrink-0 w-9 h-9 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center group-hover:bg-brand-100 group-hover:text-brand-700" aria-hidden="true">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            </span>
                            <span class="min-w-0 pt-0.5">
                                <span class="block text-xs font-medium text-slate-500 uppercase tracking-wide">Email</span>
                                <span class="text-slate-700 group-hover:text-brand-700 break-all">{{ $contact['email'] ?? '—' }}</span>
                            </span>
                        </a>
                    </li>
                    <li class="flex items-start gap-3 text-slate-600">
                        <span class="shrink-0 w-9 h-9 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center" aria-hidden="true">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        </span>
                        <span class="min-w-0 pt-0.5">
                            <span class="block text-xs font-medium text-slate-500 uppercase tracking-wide">Alamat</span>
                            <span class="text-slate-700 leading-relaxed">{{ $contact['address'] ?? '—' }}</span>
                        </span>
                    </li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="flex flex-wrap gap-3 list-none p-0 m-0">
                    <li>
                    <a href="{{ contact_instagram_url() }}" target="_blank" rel="noopener noreferrer" aria-label="Instagram" title="Instagram" class="w-10 h-10 shrink-0 rounded-full bg-white ring-1 ring-slate-200 flex items-center justify-center text-slate-700 hover:bg-brand-600 hover:text-white hover:ring-brand-600 transition-colors">

...
@section('content')
    <div class="max-w-2xl">
        <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-2xl shadow-sm">
            @csrf @method('PUT')
            <div>
                <label class="block font-medium mb-2">Judul</label>
                <input type="text" name="title" value="{{ old('title', $banner->title) }}" required class="w-full border border-slate-200 rounded-xl px-4 py-2">
            </div>
            <div>
                <label class="block font-medium mb-2">Subjudul</label>
                <input type="text" name="subtitle" value="{{ old('subtitle', $banner->subtitle) }}" class="w-full border border-slate-200 rounded-xl px-4 py-2">
            </div>
            @if($banner->image)
            <div>
                <label class="block font-medium mb-2">Gambar saat ini</label>
                <img src="{{ image_url($banner->image) }}" alt="" class="w-full max-w-md h-40 object-cover rounded-lg">
            </div>
            @endif
            <div>
            ...
            <div>
                ...
                <input type="url" name="image_url" value="{{ old('image_url', str_starts_with($banner->image ?? '', 'http') ? $banner->image : '') }}" class="w-full border border-slate-200 rounded-xl px-4 py-2">
            </div>
            <div>
                <label class="block font-medium mb-2">Urutan</label>
                <input type="number" name="order" value="{{ old('order', $banner->order) }}" class="w-full border border-slate-200 rounded-xl px-4 py-2">
            </div>
            <div>

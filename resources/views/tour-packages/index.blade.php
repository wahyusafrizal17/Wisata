    ...
    <section class="py-16 lg:py-24 bg-slate-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                @php($tp = site_setting('tour-packages', []))
                <span class="text-brand-600 font-semibold text-sm uppercase tracking-wider">{{ $tp['badge'] ?? 'Paket Wisata' }}</span>
                <h1 class="text-4xl lg:text-5xl font-bold text-slate-900 mt-2">{{ $tp['title'] ?? 'Destinasi Impian Anda' }}</h1>
                <p class="text-slate-600 mt-4 max-w-2xl mx-auto">{{ $tp['subtitle'] ?? 'Pilih paket wisata favorit. Perjalanan nyaman dengan transportasi dan akomodasi terjamin.' }}</p>
            </div>
                    ...
                    <a href="{{ route('tour-packages.show', $package->slug) }}" class="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
                        <div class="aspect-[4/3] overflow-hidden relative">
                            ...
                            <div class="absolute bottom-4 left-4 right-4 text-white">
                                ...
                                <p class="text-brand-200 font-semibold text-lg">Rp {{ number_format($package->price, 0, ',', '.') }}/org</p>
                            </div>
                        </div>
                        <div class="p-4 flex items-center justify-between">
                            <span class="text-brand-600 font-medium">Lihat Detail</span>
                            <svg class="w-5 h-5 text-brand-600 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </div>
                    </a>
                    @endforeach
                </div>

...
@section('title', 'Tambah Item Drop Off')
@section('header', 'Tambah Item Drop Off')

@section('content')
    <div class="max-w-2xl">
        <form action="{{ route('admin.drop-off-items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-2xl shadow-sm">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="number" name="price" value="{{ old('price') }}" class="w-full rounded-xl border-slate-200" min="0" step="1">
                @error('price')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block font-medium mb-2">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full rounded-xl border-slate-200">{{ old('description') }}</textarea>
                @error('description')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div class="grid sm:grid-cols-2 gap-6">
                <div>
                    <label class="block font-medium mb-2">Urutan (opsional)</label>
                    <input type="number" name="order" value="{{ old('order') }}" class="w-full rounded-xl border-slate-200">
                </div>
                <div class="flex items-end">
                    <label class="inline-flex items-center gap-2">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-brand-600 focus:ring-brand-500" checked>
